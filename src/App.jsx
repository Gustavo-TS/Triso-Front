...
function useCatalogOptions(enabled) {
  ...
  const reload = async () => {
    ...
    const [categoryResult,marketplaceResult]=await Promise.allSettled([catalogService.listCategories({admin:true}),catalogService.listMarketplaces()])
    const allCategories=categoryResult.status==='fulfilled'?categoryResult.value:[]
    const marketplaces=marketplaceResult.status==='fulfilled'?marketplaceResult.value:[]
...
function ProductVisual({ product, small = false }) {
  return <div className={small ? 'admin-product-thumb' : 'product-art'}>
    {product.imageUrl ? <img src={product.imageUrl} alt={product.name}/> : <ProductShape type={product.art}/>} 
...
function Header() {
  ...
  return <header className={`header ${sticky ? 'sticky' : ''}`}><div className="container header-inner"><Brand/><nav className="desktop-nav"><a href="#loja">Produtos</a><a href="#colecoes">Coleções</a><a href="#personalize">Personalize</a><a href="#sobre">Sobre</a></nav><div className="header-actions"><a className="admin-entry" href="?admin=1">Área administrativa</a><button className="menu-button" onClick={() => setMenu(!menu)} aria-label="Abrir menu" aria-expanded={menu}><i/><i/></button></div></div><div className={`mobile-menu ${menu ? 'open' : ''}`} aria-hidden={!menu}>{['Produtos','Coleções','Personalize','Sobre'].map((item,i)=><a key={item} href={['#loja','#colecoes','#personalize','#sobre'][i]} onClick={()=>setMenu(false)}>{item}</a>)}<a href="?admin=1">Área administrativa</a></div></header>
}
...
function Hero({ products }) {
  ...
  useEffect(() => { if (matchMedia('(prefers-reduced-motion: reduce)').matches) return; const timer = setInterval(() => setLayer(value => value >= 240 ? 1 : value + 1), 180); return () => clearInterval(timer) }, [])
  const progress = `${layer / 240 * 100}%`
  return <section className="hero" id="inicio"><div className="hero-grid"/><div className="hero-glow hero-glow-a"/><div className="hero-glow hero-glow-b"/><div className="container hero-layout"><div className="hero-copy"><div className="kicker"><span>Nova coleção</span> Forma 01 — 2026</div><h1>Design que ganha<br/><em>forma.</em> Camada<br/>por camada.</h1><p>Objetos autorais para casa, setup e rotina. Escolha sua peça e compre com segurança no seu marketplace preferido.</p><div className="hero-actions"><a className="button button-primary" href="#loja">Ver produtos <span>↘</span></a><a className="text-link" href="#personalize">Criar uma peça <span>↗</span></a></div><div className="hero-notes"><span><b>01</b> PLA premium</span><span><b>02</b> Feito no Brasil</span><span><b>03</b> Compra segura</span></div></div><div className="hero-stage"><div className="stage-label"><i/> Em impressão agora</div><div className="orbit-art printing-art">{featuredProduct?.imageUrl ? <img src={featuredProduct.imageUrl} alt={featuredProduct.name}/> : featuredProduct ? <ProductShape type={featuredProduct.art}/> : <div className="printing-placeholder"/>}<div className="orbit-shadow"/><div className="print-layer"/></div><div className="stage-meta"><div><small>PRODUTO / {featuredProduct ? String(featuredProduct.id).slice(-3).padStart(3, '0') : '---'}</small><strong>{featuredProduct?.name || 'Carregando produto'}</strong></div><div className="stage-price"><small>A partir de</small><strong>{featuredProduct ? money(featuredProduct.price) : '—'}</strong></div></div><div className="stage-progress"><span style={{ width: progress }}/></div><div className="stage-readout"><span>CAMADA <b>{layer}</b>/240</span><span>0.20 MM · PLA</span></div></div></div></section>
}
...
function Step({ number,title,children }) { return <div><span>{number}</span><p><b>{title}</b><small>{children}</small></p></div> }
function Custom() { return <section className="custom section" id="personalize"><div className="container"><div className="custom-panel"><div className="custom-copy"><span className="eyebrow">Triso Custom / Sob medida</span><h2>Sua ideia não está<br/>na prateleira?</h2><p>Mande uma referência, desenho ou arquivo 3D. A gente ajuda a transformar em uma peça única, do protótipo ao acabamento.</p><a className="button button-light" href="mailto:contato@trisostudio.com.br?subject=Quero%20criar%20uma%20peça%20personalizada">Começar um projeto <span>↗</span></a></div><div className="custom-steps"><Step number="01" title="Você envia a ideia">Referência, medidas ou arquivo</Step><Step number="02" title="Nós modelamos">Ajustes técnicos e orçamento</Step><Step number="03" title="A peça ganha forma">Impressão, acabamento e envio</Step></div></div></div></section> }
function Footer() { return <footer className="footer"><div className="container"><div className="footer-main"><Brand large/><p>Objetos autorais produzidos<br/>camada por camada em São Paulo.</p><div className="footer-links"><div><b>Loja</b><a href="#loja">Todos os produtos</a><a href="#colecoes">Coleções</a><a href="#personalize">Personalizados</a></div><div><b>Ajuda</b><a href="mailto:contato@trisostudio.com.br">Contato</a><a href="#inicio">Envios e prazos</a></div><div><b>Gestão</b><a href="?admin=1">Área administrativa →</a></div></div></div><div className="footer-bottom"><span>© 2026 Triso Studio</span><span>Design local · Produção consciente</span></div></div></footer> }

function PublicStore({ products, recordClick, productError, productsLoading, onRetryProducts }) {
  const [filter,setFilter]=useState('todos')
  const [selected,setSelected]=useState(null)
  return <><Header/><main><Hero products={products}/><Collections setFilter={setFilter}/><Shop products={products} filter={filter} setFilter={setFilter} onOpen={setSelected} error={productError} loading={productsLoading} onRetry={onRetryProducts}/><Manifesto/><Custom/></main><Footer/>{selected&&<ProductDetail product={selected} onClose={()=>setSelected(null)} onMarketplaceClick={recordClick}/>}</>
}
...
function ProductForm({ product, onSave, onClose, categoryOptions, marketplaceOptions }) {
  ...
  const [form,setForm]=useState(() => {
    ...
    const firstMarketplace=marketplaceOptions[0]
    return { ...source, categoryId:source.categoryId||categoryOptions[0]?.id||'', marketplaces: getMarketplaces(source).length ? getMarketplaces(source) : firstMarketplace ? [{ marketplaceId:firstMarketplace.id, name:firstMarketplace.name, url:'', externalProductId:'' }] : [] }
  })
  useEffect(()=>{setForm(current=>({...current,categoryId:current.categoryId||categoryOptions[0]?.id||'',marketplaces:current.marketplaces.length||!marketplaceOptions[0]?current.marketplaces:[{marketplaceId:marketplaceOptions[0].id,name:marketplaceOptions[0].name,url:'',externalProductId:''}]}))},[categoryOptions,marketplaceOptions])
  ...
  const addListing=()=>{const marketplace=marketplaceOptions[0];if(marketplace)setForm(current=>({...current,marketplaces:[...current.marketplaces,{marketplaceId:marketplace.id,name:marketplace.name,url:'',externalProductId:''}]}))}
  const removeListing=index=>setForm(current=>({...current,marketplaces:current.marketplaces.filter((_,i)=>i!==index)}))
  const submit=async e=>{e.preventDefault();const valid=form.marketplaces.filter(item=>item.marketplaceId&&item.url);try{await onSave({...form,price:Number(form.price),status:form.active?'published':'draft',marketplaces:valid,marketplace:valid[0]?.name||'',marketplaceUrl:valid[0]?.url||''})}catch(error){window.alert(error.message)}}
  return <><div className="admin-modal-backdrop"><div className="product-modal"><div className="modal-head"><div><span className="admin-kicker">PRODUTO / {product?'EDIÇÃO':'NOVO'}</span><h2>{product?'Editar produto':'Cadastrar produto'}</h2></div><button onClick={onClose}>×</button></div><form className="product-form" onSubmit={submit}><div className="form-grid"><label className="field-wide">Nome do produto<input minLength="2" maxLength="120" value={form.name} onChange={e=>set('name',e.target.value)} required/></label><label>Preço inicial (R$)<input type="number" min="0" step="0.01" value={form.price} onChange={e=>set('price',e.target.value)} required/></label><label>Categoria<select value={form.categoryId} onChange={e=>set('categoryId',e.target.value)} required><option value="" disabled>Selecione</option>{categoryOptions.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select><button className="category-create-inline" type="button" onClick={()=>setCategoryModalOpen(true)}>Gerenciar categorias</button></label><label className="field-wide">Descrição<textarea maxLength="2000" value={form.description} onChange={e=>set('description',e.target.value)} rows="3"/></label><label>Selo do produto<input value={form.badge||''} onChange={e=>set('badge',e.target.value)} placeholder="Novo, Destaque..."/></label><label>Visual padrão<select value={form.art} onChange={e=>set('art',e.target.value)}>{CATALOG_OPTIONS.visuals.map(item=><option value={item.value} key={item.value}>{item.label}</option>)}</select></label><label className="field-wide">Imagem por URL HTTPS<input type="url" pattern="https://.*" value={form.imageUrl||''} onChange={e=>set('imageUrl',e.target.value)} placeholder="https://.../produto.jpg"/></label><div className="marketplace-fields field-wide"><div className="marketplace-fields-head"><span><b>Anúncios nos marketplaces</b><small>O domínio precisa ser autorizado para o canal escolhido</small></span><button type="button" onClick={addListing} disabled={!marketplaceOptions.length||form.marketplaces.length>=10}>+ Adicionar canal</button></div>{form.marketplaces.map((listing,index)=><div className="marketplace-field-row" key={listing.id||index}><select value={listing.marketplaceId||''} onChange={e=>setListingMarketplace(index,e.target.value)} required><option value="" disabled>Marketplace</option>{marketplaceOptions.map(item=><option value={item.id} key={item.id}>{item.name}</option>)}</select><input type="url" pattern="https://.*" value={listing.url} onChange={e=>setListing(index,'url',e.target.value)} placeholder="Link HTTPS do anúncio" required/><input value={listing.externalProductId||''} onChange={e=>setListing(index,'externalProductId',e.target.value)} placeholder="ID externo (opcional)"/><button type="button" onClick={()=>removeListing(index)} aria-label="Remover canal">×</button></div>)}</div><label className="status-toggle field-wide"><input type="checkbox" checked={form.active} onChange={e=>set('active',e.target.checked)}/><i/><span><b>Produto publicado</b><small>Aparece na vitrine pública</small></span></label></div><div className="form-preview"><span>PRÉ-VISUALIZAÇÃO</span><ProductVisual product={form}/><h3>{form.name||'Nome do produto'}</h3><p>{form.marketplaces.length} canais · {form.price?money(form.price):'R$ 0,00'}</p></div><div className="modal-actions"><button type="button" className="admin-secondary" onClick={onClose}>Cancelar</button><button type="submit" className="admin-primary">Salvar produto <span>→</span></button></div></form></div></div>{categoryModalOpen&&<CategoryModal onClose={()=>setCategoryModalOpen(false)} onChanged={categoriesChanged}/>}</>
}
...
function CategoryModal({ onClose, onChanged }) {
  ...
  useEffect(()=>{load();const close=event=>event.key==='Escape'&&onClose();document.addEventListener('keydown',close);return()=>document.removeEventListener('keydown',close)},[onClose])
